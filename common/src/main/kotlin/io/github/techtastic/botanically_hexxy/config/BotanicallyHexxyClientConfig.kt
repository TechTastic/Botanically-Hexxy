package io.github.techtastic.botanically_hexxy.config
...
object BotanicallyHexxyClientConfig {
    @JvmStatic
    ...
    fun init() {
        ...
        holder.registerSaveListener { _, config ->
            BotanicallyHexxyServerConfig.holder.config = BotanicallyHexxyServerConfig.GlobalConfig(config.server)
            InteractionResult.PASS
        }

        // when we leave a server, clear our local copy of its config
        ClientPlayerEvent.CLIENT_PLAYER_QUIT.register { _ ->
            BotanicallyHexxyServerConfig.onSyncConfig(null)
        }
    }

    @Config(name = BotanicallyHexxy.MODID)
    class GlobalConfig : GlobalData() {
        @Category("server")
        @TransitiveObject
        val client = ClientConfig()

        // this should only be used inside of this class; use BotanicallyHexxyServerConfig.config to access the server-side config in other code
        @Category("server")
        @TransitiveObject
        val server = BotanicallyHexxyServerConfig.ServerConfig()
    }

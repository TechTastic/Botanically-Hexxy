package io.github.techtastic.botanically_hexxy.registry
...
typealias RegistrarEntry<T> = BotanicallyHexxyRegistrar<T>.Entry<out T>

abstract class BotanicallyHexxyRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(BotanicallyHexxy.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is BotanicallyHexxyRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
